...
import 'package:stream_provider/pages/timer/ticker.dart';
...
class Timer extends _$Timer {
  ...
  void startTimer() {
    state = AsyncData(TimerRunner(_duration));
    _tickerSubscription?.cancel();
    _tickerSubscription = _ticker.tick(ticks: _duration).listen((duration) {
      state = duration > 0
          ? AsyncData(TimerRunner(duration))
  ...
  void resumeTimer() {
    switch (state.value!) {
      case TimerPaused(:int duration):
        ...
        state = AsyncData(TimerRunner(duration));
      case _:
    }
  }
